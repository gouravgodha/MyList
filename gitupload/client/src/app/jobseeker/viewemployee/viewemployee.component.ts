      ...
      if(localStorage.getItem('users')!=null && localStorage.getItem('users')!=undefined)
      {
       this.users=JSON.parse(localStorage.getItem('users')); 
        ...
        this.route.params.subscribe(params => {
            this.getEmployeeHistory(params['emp_id']);
    ...
    getEmployeeHistory(emp_id){
        this.jobSeekerService.getEmployeeById(emp_id).subscribe(res => {
                ...
                if (res.status == 200) {
                    ...
                    this.employeeEditFrom.controls["status"].setValue(this.employee_data.status);
      ...
      downloadp()
    {
            window.open(`http://${this.employee_data.docFilePath}`, '_blank');
    }
